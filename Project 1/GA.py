...
if __name__ == "__main__":
    ...
    # draw_route(result[0]['chromosome'], city_table=city_10)
    # print(result[0]['chromosome'])
    # draw(par, result[2], result[3])
    ...
    # solution = GA_Process3(par, city_table=more_city, fixed_pos=[])
    # draw(par, solution[1], solution[2])
    # Version 2-----------------------------------
    # more_city = add_more_city(city_10, 24)
    ...
    # Version 3-----------------------------------
    # par = init_para(pop_size=200, max_gen=30, k_tournament=5, crossover_rate=0.85, mutation_rate=0.3)
    # start_end = [8, 9]
    # solution = GA_Process3(par, city_table=city_10, fixed_pos=start_end)
    # draw(par, solution[1], solution[2])
    # Version 2-----------------------------------
    # start_end = [6, 7]
    ...
    # par = init_para(pop_size=100, max_gen=30, k_tournament=5, crossover_rate=0.85, mutation_rate=0.03)
    # fixed_point = [1,2,3]
    # solution = GA_Process3(par, city_table=city_10, fixed_pos=fixed_point)
    # draw(par, solution[1], solution[2])
    # Version 2-----------------------------------
    # fixed_point = [6, 7, 8]
    ...
    # par = init_para(pop_size=100, max_gen=8, k_tournament=5, crossover_rate=0.85, mutation_rate=0.03)
    # data['label'] = label
    # label0 = data[data['label']==0]
    # label1 = data[data['label']==1]
    # label2 = data[data['label']==2]
    # solution1 = GA_Process3(par, city_table=label0, fixed_pos=[])
    # solution2 = GA_Process3(par, city_table=label1, fixed_pos=[])
    # solution3 = GA_Process3(par, city_table=label2, fixed_pos=[])
    # print(solution1[-1] + solution2[-1] + solution3[-1])
